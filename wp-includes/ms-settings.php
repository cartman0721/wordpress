...
// $base sanity check.
if ( 'BASE' == $base )
	die( /*WP_I18N_BASE_ERROR*/'<code>wp-config.php</code> 文件配置错误。<code>$base</code> 设置为 <code>BASE</code>，但应该是 <code>/</code> 或者 <code>/blogs/</code>。'/*/WP_I18N_BASE_ERROR*/ );
...
if ( !isset( $current_site ) || !isset( $current_blog ) ) {
	...
	if ( false !== strpos( $domain, ':' ) ) {
		if ( substr( $domain, -3 ) == ':80' ) {
			...
			wp_die( /*WP_I18N_NO_PORT_NUMBER*/'多站点只能工作在没有端口号的URL中。'/*/WP_I18N_NO_PORT_NUMBER*/ );
		}
	}
	...
	if ( ! $blog_id ) {
		if ( defined( 'WP_INSTALLING' ) ) {
			...
			$msg = ! $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) ? ' ' . /*WP_I18N_TABLES_MISSING*/'数据表丢失。'/*/WP_I18N_TABLES_MISSING*/ : '';
			wp_die( /*WP_I18N_NO_BLOG*/'该站点不存在。'/*/WP_I18N_NO_BLOG*/ . $msg );
		}
	}
}
