...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'该站点不存在。请尝试 <a href="%s">%s</a>。'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'本主机不存在指定的站点。如果您是站长，请查看 <a href="http://codex.wordpress.org/Debugging_a_WordPress_Network">Debugging a WordPress Network</a> 来获得帮助。'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'如果您的站点无法显示，请联系管理员。.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'如果您是管理员，请查看MySQL是否正常运行，是否所有的数据表都没有错误。'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>数据表丢失。</strong> 这意味着 MySQL 没有正常运行，WordPress 没有正确安装，或者是有人删除了 <code>%s</code>。您现在应该检查您的数据库。'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>无法找到站点 <code>%1$s</code>。</strong>搜索 <code>%3$s</code> 中的 <code>%2$s</code>。看看是否正常。'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'现在我应该怎么做？'/*WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'阅读 <a target="_blank" href="http://codex.wordpress.org/Debugging_a_WordPress_Network">bug report</a> 页面。这或许能帮助你找出问题。'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'如果您依然遇到这个问题，检查您的数据库：'/*/WP_I18N_STUCK*/ . '</p><ul>';
